...
function showTab(tabId, ev) {
  ...
  if (ev && ev.currentTarget) {
    ...
    // try to find a button with inline onclick containing the tabId (works with your HTML)
    const btn = Array.from(navButtons).find(b => {
  ...
  // lazy-load or set up tab-specific content
...
function withShuffledOptions(q) {
  // Make a shallow copy with displayOptions and index mapping
...
function selectMockAnswer(i) {
  ...
  document.querySelectorAll('#mockQuestion .option').forEach((btn, idx) => {
...
function endMockTest() {
  ...
  if (pbText) pbText.textContent = 'Test Completed';
}
